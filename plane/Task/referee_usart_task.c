  ...
  //  draw_init_all(send_id, receive_id);
    ...
    while (1)
    {
        ...
        if ((!(rc_ctrl.key & KEY_PRESSED_OFFSET_SHIFT)) && rc_ctrl.key & KEY_PRESSED_OFFSET_G)
       {
        ...
        {
            draw_servo_bull_circle(send_id, receive_id, 2);
        }
        else if (time_heart_ms % 240 == 0)
        {
            draw_pitch_number(send_id, receive_id, 2);
						//draw_pitch(send_id, receive_id, 2);
        }
        else if (time_heart_ms % 190 == 0)
        {
            draw_speed_cut_number(send_id, receive_id, 2);
        }
        else if (time_heart_ms % 50 == 0)
        {
            draw_gimbal_relative_angle_tangle(send_id, receive_id, 2);
        }
        else if (time_heart_ms % 10 == 0)
        {
